"""
Opening Range Breakout (ORB) Strategy on TQQQ

Strategy Logic:
1. Calculate opening range: first 5 minutes of trading day
2. Entry: Long if bullish 5 minute opening, Short if bearish 5 minute opening Breakout of opening range high (long) or low (short)
3. Exit: Take-profit (30% ATR) OR stop-loss (50% ATR) OR end of day
4. Stop Loss: at the opening range low (high), if Long (short)
5. Take Profit: At multiple of the stop-loss distance; default is 10x
6. Position sizing: Risk 1% or less of portfolio per trade
...
STRATEGY_NAME = "ORB_Aziz_TQQQ"
ATR_PERIOD = 14
OPENING_RANGE_MINUTES = 5
OPTIMIZATION_TAKE_PROFIT_FACTOR = 10 # take-profit at 10x risk amount
...
class ORBAzizTQQQ(TradingStrategy):
    """
    ...
    def calculate_opening_range(self, bars: list):
        ...
        self.opening_range_open = bars[0]['o']
    ...
    def on_minute_bar(self, bar: dict, current_datetime: datetime = None):
        ...
        if self.invested:
            ...
            if price > self.opening_range_open:
                ...
                position_size = self.calculate_position_size(price, self.opening_range_low)

                if position_size > 0:
                    self.entry_price = price
                    self.position_direction = 'LONG'
                    self.take_profit_price = price + OPTIMIZATION_TAKE_PROFIT_FACTOR * (price - self.opening_range_low)
                    self.stop_loss_price = self.opening_range_low
                ...
                position_size = self.calculate_position_size(price, self.opening_range_high)

                if position_size != 0:
                    self.entry_price = price
                    self.position_direction = 'SHORT'
                    self.take_profit_price = price - OPTIMIZATION_TAKE_PROFIT_FACTOR * (self.opening_range_high - price)
                    self.stop_loss_price = self.opening_range_high
